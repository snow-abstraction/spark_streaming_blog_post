...
object StockOptionMarketRisk {
  ...
  def main(args: Array[String]) {
    if (args.length < 3) {
      val usage = ("Usage: StockOptionMarketRisk <hostname> <port>"
    ...
    val stockPrices = createStockPricesStream(args(0), args(1).toInt, ssc)
    ...
    val optionValulation = idPricesValulationInfoNested.map(
      y => (y._2._2._2.instrumentID,
            StockOptionValuation(y._2._1, y._2._2._1, y._2._2._2)) )

    val optionValulationShift = optionValulation.map(
      y => (y._1, shiftMarketPrice(y._2)))

    val priceAndDelta = optionValulation.join(optionValulationShift).map(
      y => (y._1, priceAndDeltaFromOriginalAndShifted(y._2._1, y._2._2)))
  ...
  val priceAndDeltaFromOriginalAndShifted = 
        (p : StockOptionValuation, shiftedP : StockOptionValuation) => {
          assert(p.volatility == shiftedP.volatility)
          assert(p.optionParam == shiftedP.optionParam)
          ...
          val delta = 
            (price(shiftedP.underLyingMarketPrice) - optionPrice) / shiftSize
          (optionPrice, delta)
        }
